...
var MarkdownCustomAttribute = (function () {
    ...
    MarkdownCustomAttribute.prototype.parseMath = function (str) {
        // var html = katex.renderToString("c = \\pm\\sqrt{a^2 + b^2}");
        var inlineRegex = /\$\$(.*)\$\$/gmi; // Math between $$ and $$
        str = str.replace(inlineRegex, function (s, r) {
            return katex.renderToString(r, { displayMode: false });
        });
        var multilineRegex = /\\\\\[(?!(\\\\\]))([\s\S]*?)\\\\\]/gmi; // Math between \\[ and \\]
        str = str.replace(multilineRegex, function (s, ignore, r) {
